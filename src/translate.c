...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
    if (strcmp(name, "li") == 0) {
	long int imd;
        int err = translate_num(&imd, args[1], -2147483648, 2147483647);
	if (err == -1) {
	    return 0;
	}
	if (imd >= 65336) {
	    long int imd_upper = imd >> 16;
	    int imd_lower = (imd << 16) >> 16;
...
int write_rtype(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    instruction = (instruction & funct) << 26;
...
int write_addiu(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    int rs = translate_reg(args[1]);
    long int imd;
    int err = translate_num(&imd, args[2], 0, 31);
    if ((rt == -1) || (rs == -1) || (err == -1)) {
	return -1;
    }
    uint32_t instruction = 0;
    instruction = (instruction | funct) << 26;
    instruction = instruction | imd;
    instruction = instruction | (rt << 16);
    instruction = instruction | (rs << 21);
    write_inst_hex(output, instruction);
    return 0;
...
int write_lui(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    int rt = translate_reg(args[0]);
    long int imd;
    int err = translate_num(&imd, args[1], 0, 31);
    if ((rt == -1) || (err == -1)) {
	return -1;
    }
    uint32_t instruction = 0;
...
int write_mem(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    int rt = translate_reg(args[0]);
    char** reg = args;
    int off = strtol(args[1], reg, 0);
    (*reg)++;
    char* track = *reg;
    while (strcmp(")", track) != 0) {
	track++;
    }
    *track = '\0';
    int rs = translate_reg(*reg);
    if ((rt == -1) || (rs == -1)) {
	return -1;
    }
    uint32_t instruction = 0;
...
int write_jump(uint8_t funct, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* reltbl) {
    ...
    char* label = args[0];
    uint32_t instruction = 0;
    addr = get_addr_for_symbol(reltbl, label);
    if (addr == -1) {
	return -1;
    }
    instruction = (instruction | funct) << 26;
...
int write_branch(uint8_t funct, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* symtbl) {
    ...
    char* label = args[2];
    addr = get_addr_for_symbol(symtbl, label);
    if ((rs == -1) || (rt == -1) || (addr >= 65536)) {
	return -1;
    }
    uint32_t instruction = 0;
    instruction = (instruction | funct) << 26;
